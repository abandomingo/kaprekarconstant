...
def strIntoList(initialNum):
    ...
    for x in initialNum: #starts as a str
      temp = int(x) #split the characters into a list and turn them into integers
...
def calculation(topInt, lowInt, count):
    if topInt <= 1000:                          #if the top integer is less than 1000 we need to add a trailing zero 999 -> 9990
      topInt *= 10                              #do this by muliplying by 10
    
    initialNum = topInt - lowInt                #subtract the two and make it our 'new' initial number
...
def checkResult(initialNum, count):
    if initialNum == 6174:
      txt = "It took {0} iterations until you reached kaprekar's constant"  #we reached the const
      print(txt.format(count))
    elif count > 50:                                                        #lazy stopper 
      print("The number may have been inputed wrong LOL ")
    else:
      kaprekar(str(initialNum),count)                                       #recurse using new initial number and up the count
